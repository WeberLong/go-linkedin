...
// 生成32位md5字串
func (a API) GetMd5String(s string) string {
	h := md5.New()
	h.Write([]byte(s))
...
func (a API) GetUniqueId() string {
	...
	return a.GetMd5String(base64.URLEncoding.EncodeToString(b))
}

// func (a *API) SetState() {
// 	a.state = a.GetUniqueId()
// }
...
func (a API) AuthUrl(state string, redirect_url string) (url string) {
	return "https://www.linkedin.com/uas/oauth2/authorization?response_type=code&client_id=" + a.oauth_key + "&state=" + state + "&redirect_uri=" + redirect_url
}
...
func (a *API) RetrieveAccessToken(client *http.Client, code string, redirect_url string) (t string, e error) {
	...
	// send the request
	resp, err := client.Get("https://www.linkedin.com/uas/oauth2/accessToken?grant_type=authorization_code&code=" + code + "&redirect_uri=" + redirect_url + "&client_id=" + a.oauth_key + "&client_secret=" + a.oauth_secret)
